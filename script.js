...
/* ===== CalculatorWorks URL Consistency Cleanup ===== */
(function(){function cleanInternalUrl(url){if(!url||url.startsWith('http')||url.startsWith('#')||url.startsWith('mailto:')||url.startsWith('tel:'))return url;if(url==='index.html'||url==='./index.html')return '/';return url.replace(/^\.\//,'').replace(/\.html(?=($|[?#]))/,'');}document.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('a[href]').forEach(function(a){var oldHref=a.getAttribute('href');var newHref=cleanInternalUrl(oldHref);if(newHref&&newHref!==oldHref){a.setAttribute('href',newHref);}});});})();
