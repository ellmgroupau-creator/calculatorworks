...
/* ===== CalculatorWorks Trust + Data Authority Layer ===== */

(function(){
'use strict';

function addTrustInsights(){
  if(document.querySelector('.cw-methodology')) return;

  const page=(location.pathname.split('/').filter(Boolean).pop()||'index').replace(/\.html$/,'');

  let html='';

  if(/mortgage|loan|refinance/.test(page)){
    html=`
    <section class="cw-methodology">
      <h2>How mortgage estimates are calculated</h2>
      <p>Mortgage repayment estimates are generally based on principal, interest rate, repayment frequency and loan term assumptions.</p>
      <div class="cw-methodology-grid">
        <div class="cw-methodology-card"><strong>Interest assumptions</strong><span>Results assume the rate remains constant unless otherwise stated.</span></div>
        <div class="cw-methodology-card"><strong>Repayment frequency</strong><span>Monthly repayment estimates may differ from weekly or fortnightly schedules.</span></div>
        <div class="cw-methodology-card"><strong>Extra repayments</strong><span>Additional payments can reduce interest and shorten payoff time.</span></div>
      </div>
      <table class="cw-benchmark-table">
        <thead><tr><th>Example</th><th>Typical observation</th></tr></thead>
        <tbody>
          <tr><td>Lower rate</td><td>Usually reduces total borrowing cost</td></tr>
          <tr><td>Shorter term</td><td>Higher monthly payments but less total interest</td></tr>
          <tr><td>Extra repayment</td><td>Can shorten payoff time materially</td></tr>
        </tbody>
      </table>
      <div class="cw-disclaimer"><strong>Important:</strong> Mortgage estimates do not replace professional financial or lending advice. Taxes, fees, insurance and local lending rules may change real-world outcomes.</div>
    </section>`;
  }

  if(/compound-interest|retirement|investment|savings/.test(page)){
    html=`
    <section class="cw-methodology">
      <h2>How growth projections work</h2>
      <p>Investment and savings projections estimate growth using contribution assumptions, compounding frequency and average return rates.</p>
      <div class="cw-methodology-grid">
        <div class="cw-methodology-card"><strong>Average return assumptions</strong><span>Long-term averages can differ significantly from year-to-year performance.</span></div>
        <div class="cw-methodology-card"><strong>Inflation</strong><span>Future purchasing power may differ from nominal balances.</span></div>
        <div class="cw-methodology-card"><strong>Contribution discipline</strong><span>Regular contributions often have a large impact over time.</span></div>
      </div>
      <table class="cw-benchmark-table">
        <thead><tr><th>Projection factor</th><th>Potential impact</th></tr></thead>
        <tbody>
          <tr><td>Higher contribution</td><td>Can materially improve long-term outcomes</td></tr>
          <tr><td>Longer time horizon</td><td>Allows compounding more time to work</td></tr>
          <tr><td>Lower fees</td><td>Improves net long-term growth</td></tr>
        </tbody>
      </table>
      <div class="cw-disclaimer"><strong>Important:</strong> Investment returns are not guaranteed and actual outcomes may vary due to fees, taxes, inflation and market conditions.</div>
    </section>`;
  }
  ...
  if(/salary|paycheck|tax/.test(page)){
    html=`
    <section class="cw-methodology">
      <h2>How salary and paycheck estimates work</h2>
      <p>Salary and paycheck estimates are typically based on gross income assumptions before regional tax and deduction differences are applied.</p>
      <div class="cw-methodology-grid">
        <div class="cw-methodology-card"><strong>Gross vs net</strong><span>Take-home pay can differ materially from gross salary.</span></div>
        <div class="cw-methodology-card"><strong>Regional tax rules</strong><span>Tax systems vary by country and jurisdiction.</span></div>
        <div class="cw-methodology-card"><strong>Hours worked</strong><span>Longer hours can reduce effective hourly value.</span></div>
      </div>
      <div class="cw-disclaimer"><strong>Important:</strong> Tax and payroll estimates are educational only and may not reflect current local tax law.</div>
    </section>`;
  }
  ...
  const target=document.querySelector('.cw-depth-engine') ||
               document.querySelector('.cw-report-layer') ||
               document.querySelector('.cw-ai-engine') ||
               document.querySelector('.calculator-card');
...
if(document.readyState==='loading'){
  document.addEventListener('DOMContentLoaded',addTrustInsights);
}else{
  addTrustInsights();
}
